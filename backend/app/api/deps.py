...
def get_user_credentials(
    username: str = Depends(get_username), db: Session = Depends(get_session)
) -> UserCredentials:
    user_credentials = (
        db.query(UserCredentials).where(UserCredentials.username == username).scalar()
    )
    print(user_credentials)
    if not user_credentials:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="Username does not exist"
        )
