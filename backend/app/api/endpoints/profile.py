...
async def update_profile(profile: schemas.UserProfile, user_credentials: models.UserCredentials = Depends(deps.get_user_credentials), db: Session = Depends(deps.get_session)):
...
async def get_profile_details_by_username(user_credentials: models.UserCredentials = Depends(deps.get_user_credentials), client_information: models.ClientInformation = Depends(deps.get_client_information)):
    return {"username": user_credentials.username,
